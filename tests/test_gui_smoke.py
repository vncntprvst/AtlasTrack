...
@pytest.mark.qt
def test_landmark_pair_clicking_records_atlas_anchor_then_tissue_target(qtbot) -> None:
    """HERBS-style entry: click 1 is the atlas anchor, click 2 the tissue target.

    Both halves must land in ONE row of the same points layer the drag workflow
    uses - data = target, features = source. The subtle part is that appending a
    point makes _on_landmark_data anchor it where it was dropped, so the pair code
    has to write the atlas side back over that; if it ever stopped doing so, every
    pair would silently become a zero-displacement point and the warp would be the
    identity.
    """
    import napari
    ...
    try:
        ...
        layer = viewer.add_points(
            np.zeros((0, 2)), name="Atlas landmarks 0",
            features={"sy": np.zeros(0), "sx": np.zeros(0)}, ndim=2,
        )
        panel._landmark_idx = 0
        panel._lm_prev_data = np.zeros((0, 2))
        layer.events.data.connect(panel._on_landmark_data)  # as the real session does

        panel._record_pair_click(100.0, 200.0)  # click 1: on the atlas outline
        assert panel._pair_anchor == (100.0, 200.0)
        assert PAIR_ANCHOR_LAYER in viewer.layers, "the pending anchor must be visible"
        assert len(np.asarray(layer.data).reshape(-1, 2)) == 0, "no point until paired"

        panel._record_pair_click(130.0, 240.0)  # click 2: the matching tissue spot

        data = np.asarray(layer.data, dtype=float).reshape(-1, 2)
        assert len(data) == 1
        assert tuple(data[0]) == (130.0, 240.0)
        assert float(layer.features["sy"][0]) == 100.0
        assert float(layer.features["sx"][0]) == 200.0
        assert panel._pair_anchor is None
        assert PAIR_ANCHOR_LAYER not in viewer.layers
    finally:
        viewer.close()


@pytest.mark.qt
def test_leaving_landmark_edit_also_leaves_pair_mode(qtbot) -> None:
    """A live viewer hook that outlives its session would keep eating clicks."""
    import napari
    ...
    try:
        panel = RegisterPanelWidget(WorkflowState(), viewer)
        qtbot.addWidget(panel)
        panel._pair_cb = panel._pair_click_callback
        viewer.mouse_drag_callbacks.append(panel._pair_cb)
        panel._set_pair_checked(True)

        panel._exit_landmark_edit()

        assert panel._pair_cb is None
        assert panel._pair_click_callback not in viewer.mouse_drag_callbacks
        assert not panel._lm_pair_btn.isChecked()
    finally:
        viewer.close()
...
def test_build_panel_constructs_full_app(qtbot) -> None:
    ...
    try:
        ...
        # Only our Project / Settings / Help menus are visible; napari's defaults
        # (File / View / Plugins / Window / Help) are hidden - including its own
        # Help, which shares a title with ours and so is matched by identity.
        visible = [
            (a.menu().title() if a.menu() else a.text()).replace("&", "")
            for a in viewer.window._qt_window.menuBar().actions()
            if a.isVisible()
        ]
        assert set(visible) == {"Project", "Settings", "Help"}, visible
